...
const withdraw = async () => {
  ...
  if (parseInt(withdrawAmount.wAmount) <= myBalance) {
    let amountLeft = myBalance - parseInt(withdrawAmount.wAmount);
      console.log(
        chalk.green(`Successfully withdrawn ${withdrawAmount.wAmount}. Remaining balance is ${amountLeft}`)
      );
    } else {
...
const deposit = async () => {
  ...
  console.log(
    chalk.green(`Successfully deposited ${depositAmount.dAmount}. Your new balance is ${myBalance}`)
  );
};
